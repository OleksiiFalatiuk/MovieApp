    ...
    private suspend fun loadActors(): List<Actor> = withContext(Dispatchers.IO) {
        ...
        jsonActors.map { jsonActor ->
            Actor(
                id = jsonActor.id,
                name = jsonActor.name,
                imageRes = jsonActor.profilePicture
            )
        }
    }
    ...
    private fun parseMovies(
        ...
        return jsonMovies.map { jsonMovie ->
            Movie(
                ...
                avatar = jsonMovie.posterPicture,
                detailImageRes = jsonMovie.backdropPicture,
                rating = (jsonMovie.ratings / 2).toInt(),
                review = jsonMovie.votesCount,
                years = if (jsonMovie.adult) 16 else 13,
                time = jsonMovie.runtime,
                genre = jsonMovie.genreIds.map { id ->
